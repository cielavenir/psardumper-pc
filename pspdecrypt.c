...
typedef struct
{
    ...
    const u8* key; // "step1_result" use for XOR step
    u8 code;
    u8 codeExtra;
} TAG_INFO;
...
static TAG_INFO g_tagInfo[] =
{
	...
	{ 0x06000000, g_keyDEVPSAR, 0x49 },
	{ 0x07000000, (u8*)g_key_INDEXDAT1xx, 0x4A },
	{ 0x08000000, (u8*)g_keyEBOOT1xx, 0x4B },
	{ 0x09000000, (u8*)g_key_GAMESHARE1xx, 0x4C },
	{ 0x0A000000, (u8*)g_key_GAMESHAREDEMO_150, 0x4D },
	{ 0x0B000000, (u8*)g_keyUPDATER, 0x4E },
	{ 0x0C000000, (u8*)g_keyDEMOS27X, 0x4F },
	{ 0x0E000000, g_keyPSAR, 0x51},
	{ 0x0F000000, (u8*)g_keyMEIMG250, 0x52 },
	{ 0x862648D1, (u8*)g_keyMEIMG260, 0x52, 0x52 },
	{ 0x4467415d, (u8*)g_key44, 0x59, 0x59 },
	{ 0x207bbf2f, (u8*)g_key20, 0x5A, 0x5A },
	{ 0x3ace4dce, (u8*)g_key3A, 0x5B, 0x5B },
	{ 0xC0CB167C, (u8*)g_keyEBOOT2xx, 0x5D, 0x5D },
	{ 0x207BBF2F, (u8*)g_keyUNK1, 0x5A, 0x5A },
	{ 0xBB67C59F, (u8*)g_key_GAMESHARE2xx, 0x5E, 0x5E },
	{ 0x7F24BDCD, (u8*)g_demokeys_280, 0x60, 0x60 }
};
...
static const u8 key_102DC8AF_1[] = {0x02, 0x2B, 0x67, 0x21, 0xE7, 0x86, 0xAD, 0x91, 0x73, 0xBC, 0xC9, 0xDE, 0xC5, 0x7A, 0x13, 0xA4};
...
static TAG_INFO2 g_tagInfo2[] =
{
	...
	{ 0x63BAB403, key_102DC8AF_1, 0x51},
...
int pspDecompress(const u8 *inbuf, u8 *outbuf, u32 outcapacity)
{
	...
	if (inbuf[0] == 0x1F && inbuf[1] == 0x8B) 
	{
		...
		retsize = sceKernelDeflateDecompress(outbuf, outcapacity, (u8*)inbuf, NULL); // actually gzip
	}
	else if (memcmp(inbuf, "2RLZ", 4) == 0) 
	{
		retsize = LZRDecompress(outbuf, outcapacity, (u8*)inbuf+4, NULL);
	}
	else if (memcmp(inbuf, "KL4E", 4) == 0)
	{
		retsize = decompress_kle(outbuf, outcapacity, (u8*)inbuf+4, NULL, 1);
	}
	else if (memcmp(inbuf, "KL3E", 4) == 0) 
	{
		retsize = decompress_kle(outbuf, outcapacity, (u8*)inbuf+4, NULL, 0);
	}
	else
	{
		retsize = -1;
